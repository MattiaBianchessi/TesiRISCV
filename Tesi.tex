...
\end{quote}\todo{atrent: non so se sta bene in una tesi di informatica... a meno di non introdurre il motivo per cui lo spieghi}

\todo{atrent: non va lo spazio tra le parentesi a il contenuto}
\todo{atrent: non va lo spazio prima delle virgole}
...
\todo{atrent: non servono i '\\' a fine paragrafo}
...
\todo{atrent: il tono di questa intro è troppo "giornalistico" per una tesi di informatica}
...
\todo{axj: non mi è chiaro il senso di "svilupporare il proprio hardware per seguire il software}
...
\todo{atrent: la terminologia tecnica in inglese, se non è assorbita dall'italiano va messa in corsivo}

\subsection{Nomi delle estensioni}\todo{atrent: spiegare cosa si intende con estensione}
...
\section{ISA Base}\todo{atrent: ha senso entrare così in dettaglio? (mi riferisco alle sezioni da qui fino alla "altri set") se sì va motivato}
...
\end{quote}
\todo{axj: nella citazione si parla in inglese IS base, che può andare bene, ma in primo luogo offrirei una traduzione in Italiano. Nella sezione successiva parli sempre di RV32I ma solo in termini di registri. Al posto di "caratterizzato" direi "include"}
\section{RV32I}
...
\todo{axj: parli di registri sorgente e destinazione, ma fino ad  ora si sono visti solo nell'immagine}
...
\todo{axj: Onestamente, io che conosco l'argomento ho fatto fatica a capire di cosa parli. Non dico che ci sia un errore, ma che forse sei stato un filo stringato nella forma}
...
\todo{axj: Pare a me o tutta la parte precedente arriva quasi uguale da qui (vedi commento) ?}
% (https://opencores.org/websvn/filedetails?repname=neorv32&path=%2Fneorv32%2Ftrunk%2Fdocs%2Freferences%2Friscv-privileged.pdf&rev=66)? 
...
\todo{axj: citando il buon Rovazzi, è tutto molto interessante, ma forse eccessivo. Come diceva Andrea, a meno di fare un confronto totale con l'architettura di confronto, cosa che non raccomando, questo corso accelerato di architetture degli elaboratori è un po' pesante e senza un reale obiettivo}
