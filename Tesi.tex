...
Oggigiorno sono presenti numerosi processori ognuno caratterizzato da un insieme di istruzioni che definiscono le istruzioni\todo{ripetizione} che, un processore,  può eseguire.  RISC-V ,\todo{mai lo spazio prima della virgola} con un \textit{istruction set} basato su un approccio \textit{reduced instruction set computer}\cite{RISCOverview}, sta facendo parlare di sè
\cite{RISCVHistory}.
...
%	2- storia  \\
RISC-V è una ISA basato sul principio RISC nato come progetto accademico.  Nel 2010, a Berkeley, il progetto inizio diretto dal prof. David Patterson finanziato da \textit{Intel } e \textit{Microsoft} e da alcune aziende Californiane. La prima pubblicazione è dell' anno\todo{mai spazio dopo apostrofo} successivo. Nel 2015 viene fondata  \textbf{RISC-V Foudation} un azienda no-profit che controlla la direzione di sviluppo di RISC-V. Nel 2018 viene annunciata una collaborazione tra l'azienda e \textit{Linux Foundation} con il quale si supporta lo sviluppo del progetto RISC-V \cite{RISCOrgHistory}.


\section{Gli obbiettivi di RISC-V}
Durante la fase di progettazione i progettisti hanno voluto mettere nero su bianco gli obbiettivi di RISC-V. Gli obbiettivi\todo{ripetizione} dichiarati , nell'introduzione, della specifica dell' ISA user-mode sono:
\begin{itemize}
	...
	\item un ISA non specifica per una micro-architettura o una tecnologia ma che permetta un implementazione efficiente per ogni implementazione.\todo{ripetizione}
	\item modulare , organizzata in ISA più piccole con la possibilità di usare estensioni.
	\item supporto per lo standard floating-point 2008 IEEE-754.\todo{link}
	\item supporto delle estensioni .\todo{mai spazio prima del punto, spiega estensioni in che senso}
	\item spazio di indirizzamento 32 e 64 bit.
	\item supporto a delle implementazioni multi core e manycore\todo{spiega termine manycore o link o footnote} sia eterogenei sia omogenei.
...
\section{Panoramica}
L'ISA RISC-V è un architettura load-store con solo 49 istruzioni base. L'ISA supporta sistemi di memoria \textit{little-endian} che \textit{big-endian}. Le istruzioni sono organizzate in pacchetti da 16-bit memorizzati in maniera \textit{little-endian} indipendentemente dal \textit{endianness}\todo{per me endianness è femminile} del sistema.  Ogni pacchetto ha nei bit meno significativi i bit per la codifica della lunghezza in questo modo il sistema lo decodifica, in questo modo le istruzioni di lunghezza variabile sono decodificate velocemente.

Le celle della memoria principale sono di lunghezza variabile a seconda dell' ISA base scelto, il numero di celle di memoria è XLEN e la dimensione della singola cella è di $2^{XLEN - 1}$. Ad esempio l'ISA RV32I ha un XLEN di 32.Lo\todo{sempre spazio dopo il punto} spazio di indirizzamento è circolare quindi l'errore di overflow non c'è in quanto i calcoli degli indirizzi vengono scalati in modo adeguato dividendo per un modulo adeguato , sfruttando dunque la caratteristica circolare \cite{ISA}.
...
Essendo un’architettura RISC e dunque avendo tutte le istruzioni a lunghezza fissa, non è possibile avere direttamente costanti o indirizzi superiori alla lunghezza assegnata al campo dell’istruzione. Per ovviare a questo problema si usano quelle che sono dette modalità di indirizzamento:\todo{due punti??? segue una bullet list o simili...}
...
\todo{footnote PRIMA del punto, ci sono due punti}

%%
% Capitolo 2
% BASE -> ESTENSIONI
% 
\chapter{ISA RISC-V}\todo{atrent: riletto, ok struttura, ma pesantemente da sistemare ortografia}
Prima di discutere dei risultati ottenuti presentiamo L'\textit{Instruction Set Architecture} o \textbf{ISA}.
%BASE ED ESTENSIONE
\section{Base}
RISC-V prevede un nucleo di istruzioni di base mediante le quali si può supportare dei sistemi funzionanti.\todo{eh?} Esistono diversi nuclei base denominati a seconda di quanti bit utilizza il sistema.  Esistono\todo{ripetizione} 4 basi:
\begin{itemize}
	\item RV32I, che ha lo spazio di indirizzamento di\todo{a} 32 bit.  
...
%%?
\section{Estensioni}
Se si utilizza solo una base si hanno solo delle funzionalità basilari, per questo motivo vengono introdotte le estensioni. Usare un estensione\todo{apostrofo!} significa estendere le funzionalità e aggiungere il supporto per determinate azioni.Di\todo{spazio dopo punto} seguito vengono riportate alcune estensioni:
\begin{itemize}
...
\todo{da qui in poi smetto di correggere ortografia, però applica davvero un correttore!}
...
\section{Descrizione}
Un compilatore è un programma che trasforma il codice sorgente in linguaggio macchina. Il motivo più comune\todo{quale sarebbe il meno comune?} per trasformare il codice sorgente è creare un programma eseguibile su una determinata macchina. Il processo di compilazione prevede diverse fasi. La prima prevede un analisi lessicale da cui vengono generati dei token. La seconda ,utilizzando i token, prevede un analisi sintattica e infine un analisi semantica dopo la quale viene generato il codice un codice intermedio. Questo codice intermedio attraversa una fase di ottimizzazione e infine viene generato il codice target \cite{GCC} \cite{LLVM}.
...
\section{Storia}
Il primo compilatore teorico fu pensato da Corrado Böhm che nel 1951 che lo sviluppò per la sua tesi di dottorato.  Una  prima implementazione di un compilatore è dovuta a Grace Hopper  che ha anche coniato il termine ``compilatore''.  Il primo compilatore , chiamato \textit{A-0 System} ,  funzionava come caricatore o linker\todo{allora non era un compilatore vero e proprio, elabora meglio} , non come i moderni compilatori. E' importante menzionare che una versione successiva del \textit{A-0 System} (la versione A-2 datata 1953)fu rilasciato ai clienti con lo scopo di sviluppare dei miglioramenti all UNIVAC, quindi possiamo considerarlo come il primo software libero e open source\todo{elabora, aveva una licenza considerabile libera?!?} della storia dell'informatica \cite{SystemA0,openSource1,openOrg,stallman2003software}.\todo{le citazioni vanno più vicine a dove le usi, non tutte in fondo}

\section{Cross-compilazione}
\label{Sec:Cross}
Con il termine \textbf{cross-compilazione} si intende la tecnica mediante la quale si utilizza un compilatore per compilare il codice sorgente e il codice generato ha come target un architettura diversa da quella su cui viene utilizzato il cross - compilatore.  Alcuni esempi di una cross-compilazione può essere la compilazione di un applicazione per un sistema embedded. \todo{elabora meglio}
...
Oltre a generare programmi eseguibili ad alte prestazioni, i compilatori devono anche avere prestazioni elevate. Un progetto software di grandi dimensioni  può contenere da centinaia a migliaia di singole unità di traduzione\todo{spiega termine}. Ogni unità di traduzione può contenere migliaia di righe di codice.  Oltre all efficienza delle prestazioni del codice generato si deve tener conto anche del tempo in cui si genera il codice.
...
Il primo benchmark citato in letteratura prende il nome di Whetstone i suoi autori H.J. Curnow e B.A. Wichmann svilupparono un programma con lo scopo di valutare le prestazioni in virgola mobile.  Pubblicato nel 1976 questo benchmark fu il primo esempio di benchmark sintetico. La sua semplicità è data dalla sua organizzazione in moduli ognuno specializzato per un aspetto.
    ...
    Un altro benchmark simile è il famoso dhrystone, questo benchmark sintetico pubblicato nel 1984 ha lo scopo di valutare le prestazioni dei numeri interi\todo{le prestazioni DEI numeri interi?} nel sistema\cite{DhrystoneWP}.
    Tutti i programmi sviluppati prima dell introduzione di dhrystone erano pensati e scritti per valutare un unico aspetto. Il primo tentativo di avere un programma che valutasse più aspetti ci fu nel 1986. Il benchmark si chiama Livermore loops., questo benchmark  è composto da ventiquattro loop interni che valutano aspetti differenti. Il progetto è pensato per il \textit{parallel computers}\cite{LivermoreInfo}. \todo{sembra una pessima traduzione letterale dall'inglese...}
...
CoreMark è un benchmark sintetico, come Dhrystone e ,come Dhrystone, CoreMark è piccolo, portatile,gratuito\todo{libero?} ma ,a differenza di Dhrystone, CoreMark ha regole di esecuzione e reporting specifiche ed è stato progettato per evitare aspetti problematici di Dhrystone. Mentre Dhrystone risulta un benchmark del compilatore CoreMark si focalizza su le capacita di lavoro della MCU o di una CPU \cite{analysis_EEMBC}.
...
\todo{atrent: non lo metterei come capitolo a parte, ma come sezione del precedente}
...
\todo{non avevi citato anche un raspberry?}
...
\end{table}\todo{metterei anche i singoli risultati}
...
Device Name and Revision &
  ...
  Broadcom BCM2835 (Raspberry Pi; 256MB) &
  D1 &
  Allwinner A20 &
  X1000E &
  Xilinx XC7Z020 Dual Core ARM Cortex-A9 MPcore &
  HiFive Unleashed \\ \hline
Processor Operating Frequency {[}MHz{]} & 2,145.0            & 1,200.0       & 700.00   & 1,000.0    & 1,000.0       & 1,000.0            & 667.00             & 1,500.0  \\ \hline
Number of Processor Cores               & 4                  & 4             & 1        & 1          & 2             & 1                  & 2                  & 4        \\ \hline
Memory configuration                    & HEAP               & LPDDR2 900MHz & Stack    & DDR3       & DDR3          & Internal LPDDR2    & DDR3 533Mhz (Heap) & DDR4     \\ \hline
Compiler Name and Version               & GCC10.2.1 20210110 & GCC 7.2.1     & GCC4.6.3 & GCC 10.2.1 & GCC 4.6.3     & GCC10.2.1 20210110 & GCC4.6.1           & GCC8.3.0 \\ \hline
Compiler Flags &
  -O2 -DMULTITHREAD=4 -DUSE\_PTHREAD -DPERFORMANCE\_RUN=1 -lrt -pthread &
  -O2 &
  -O2 -DPERFORMANCE\_RUN=1 &
  -O2 -DPERFORMANCE\_RUN=1 -DITERATIONS=1000 &
  -O2 -DPERFORMANCE\_RUN=1 -lrt &
  -EL -march=mips32r5 -Ofast -DPERFORMANCE\_RUN=1 &
  -O2 -lrt &
  -O2 -DPERFORMANCE\_RUN=1 -lrt \\ \hline
Operating System Name and Version &
  RaspberryPi OS &
  Arch Linux 4.9.73-2-ARCH &
  Linux raspberrypi 3.6.11-rpi-aufs \#1 PREEMPT Sun Dec 30 22:31:28 CET 2012 armv6l GNU/Linux &
  Tina Linux &
  Bananian Linux 15.04 &
  Linux 4.4.94 &
  Linux kernel 3.3.0 &
  Linux HiFiveU 4.19.0 \\ \hline
Parallel Execution                      & 4:Pthreads         & 4:PThreads    & -        & -          & 2:Proprietary & -                  & -                  & -        \\ \hline
CoreMark                                & 48,626             & 15,364        & 1,303.8  & 2,240.8    & 2,086.2       & 2,231.1            & 2,256.1            & 3,020.5  \\ \hline
CoreMark/MHz                            & 22.670             & 12.803        & 1.8625   & 2.2408     & 2.0862        & 2.2311             & 3.3825             & 2.0133   \\ \hline
\end{tabular}
\end{table}
%tabella completa
\end{comment}
...
\todo{atrent: c'è un problema di struttura, "analisi codice assembly" contiene "operazioni", ecc. tolgo "operazioni"}
%\section{Operazioni}
...
La funzione è una semplice funzione scritta in C che dato un numero di tipo intero restituisce il numero sommato a ventitré. \todo{valore a caso o c'è un motivo?}
...
\section{Sorting}\todo{questo va separato bene dal precedente?}
...
\todo{esatto, banale e ovvio, forse non vale nemmeno la pena di dirlo}
