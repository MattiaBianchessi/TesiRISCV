...
\begin{figure}[h!]
	...
	\cite{ISA}
\end{figure}
\todo{atrent: il cite fuori dalla caption?}
...
\subsection{Storia}
Il primo benchmark citato in letteratura prende il nome di Whetstone i suoi autori H.J. Curnow e B.A. Wichmann svilupparono un programma con lo scopo di valutare le prestazioni in virgola mobile.  Pubblicato nel 1976 questo benchmark sintetico è organizzato in moduli ognuno specializzato per un aspetto.
    Un altro benchmark è LINPACK che risale al 1979(non pensato per diventare un benchmark). Questo benchmark utilizza una libreria \textit{BLAS (Basic Linear Algebra Subprograms) } per esegui operazioni su vettori e su matrici per risolvere sistemi lineari. Lo scopo originale era quello di valutare le prestazioni in virgola mobile dei supercomputer dell epoca ma poi fu utilizzato anche su architetture meno potenti. \cite{LinpackSite}\todo{atrent: i cite vanno dentro la frase, non dopo il punto}
    ...
    Fino all introduzione di dhrystone il benchmark era utilizzato per valutare un singolo aspetto dell architettura ma nel 1986 ci fu un tentativo di avere un unico programma che valutasse più aspetti. Il benchmark si chiama Livermore loops. ed è composto da ventiquattro loop interni che valutano aspetti differenti. Il progetto è pensato per il \textit{parallel computers}\cite{LivermoreInfo}. 
    
Una prime raccolte di test degna di nota è la raccolta prodotta da \textit{Stanford University}
...
La scheda di sviluppo D1-H viene fornita con il sistema Tina Linux. Il kernel fornito  adattato al kernel Linux 5.4. La board fornisce il supporto di base e gestione delle risorse hardware del dispositivo. Ulteriori informazioni sono disponibili sul sito della \href{https://d1.docs.aw-ol.com/study/study_1tina/}{board}.\todo{atrent: nel pdf il link così non si vede, è solo cliccabile, metti una footnote col link esplicito E cliccabile}
...
I codice sorgente è disponibile nella repository Github{RepoCoreMark}.\todo{atrent: io ho sempre dato "repo" per maschile... metteri inoltre il link}
...
\section{MultOrShift}\todo{atrent: questo è uno standard o un prog atto da te? va specificato}
