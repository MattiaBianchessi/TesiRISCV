...
\subsection{Storia}
Il primo benchmark citato in letteratura prende il nome di Whetstone i suoi autori H.J. Curnow and B.A. Wichmann svilupparono un programma per valutare le prestazioni in virgola mobile.  Pubblicato nel 1976 questo benchmark sintetico è organizzato in moduli ognuno specializzato per un aspetto .
    Un altro benchmark è LINPACK che risale al 1977, originariamente non pensato per diventare un benchmark. Questo benchmark utilizza una libreria \textit{BLAS (Basic Linear Algebra Subprograms) } per esegui operazioni su vettori e su matrici per risolvere sistemi lineari. Lo scopo è quello di valutare le prestazioni in virgola mobile. \cite{LinpackSite}
    ...
    Fin ora i benchmark prodotti furono per un unico scopo,  nel 1986 fu pubblicato Livermore loops. Un benchmark contenente 24 loop interni che valutano aspetti differenti. L' autore F.H. McMahon gli ha organizzati in una suite di test e ha aggiunto dei codici per la misurazione del tempo di esecuzione.\cite{LivermoreInfo} 
Una raccolta degna di nota è la raccolta prodotta da \textit{Stanford University}
e \textit{the University of California,  Berkeley} quando in corrispondenza con la progettazione del primo sistema RISC furono raccolti di piccoli programmi. Questi programmi erano un programma di permutazioni, torre di Hanoi, otto regine, moltiplicazione di matrici, puzzle, quicksort, bubblesort,treesort, moltiplicazione di matrici con valori in virgola mobile e FFT. (otto con valori interi, 2 per valori float). L'importanza di questo \textit{Stanford Small Programs Benchmark Set} è dovuta al primo confronto tra architetture RISC e CISC. \cite{CommonBench}

    Solamente nel 1988 fu fondata una cooperativa con lo scopo di produrre benchmark validi lo SPEC.\cite{SPECSite}% imparziali, significativi e pertinenti. 
    ...
    Uno dei suoi prodotti piu famosi il benchmark chiamato CoreMark pubblicato nel 2009. %sezione core Mark
...
\todo{atrent: da qualche parte spieghi la scelta degli snippet di test? cioè perché proprio quelli, tipicamente dovresti basarti su letteratura per decidere cosa testare. Esistono suite di benchmark standardizzate? Penso proprio di sì, dovresti citarle e confrontarti con esse. Anni fa ricordo che BYTE pubblicava i benchmark dei processori usando delle suite di test standard. cfr. anche \url{https://cpu.userbenchmark.com} o anche \url{https://www.cpu-monkey.com}}


\newpage
\section{CoreMark}
Per citare il sito ufficiale: \textit{CoreMark is a simple, yet sophisticated benchmark that is designed specifically to test the functionality of a processor core. Running CoreMark produces a single-number score allowing users to make quick comparisons between processors}. 
...
CoreMark è un benchmark sintetico, come Dhrystone. Come Dhrystone, CoreMark è piccolo, portatile,gratuito ma ,a differenza di Dhrystone, CoreMark ha regole di esecuzione e reporting specifiche ed è stato progettato per evitare aspetti problematici di Dhrystone. Mentre Dhrystone risulta un benchmark del compilatore CoreMark si focalizza su le capacita di lavoro della MCU o di una CPU.\cite{analysis_EEMBC}
I codice sorgente è disponibile nella repository Github{RepoCoreMark}. 
...
aritmetiche con lo shift. Il programma genera un array di interi di
...
moltiplicazione tramite shift, divisione tramite shift). Lo scopo di questo programma è osservare la differenza delle implementazioni delle operazioni a livello assembly compilato con la \textit{tool-chain} di RISCV e confrontarle con le stesse operazioni compilate per ARM.
...
\end{lstlisting}
\captionof{lstlisting}{compilazione CoreMark}
...
&
  ...
  X1000E 
  \\ \hline
CM      & 48,626             & 15,364        & 1,303.8  & 2,240.8    & 2,086.2       & 2,231.1             \\ \hline
CM/MHz                            & 22.670             & 12.803        & 1.8625   & 2.2408     & 2.0862        & 2.2311               \\ \hline
\end{tabular}
\end{table}
...
% Risultati CoreMark
% Risultati linpack
% Codice comparato: RISCV piu linee di codice che ARM
% commentare
